...
const content = new ProjectContent({
    ...
    content_short: {
        ...
        Celem projektu było stworzenie stacji roboczej z wykożystaniem edukacyjnego robota ASTORINO, aby zaprezentować jego możliwości.
    ...
    content: {
        ...
        <ul>
            ...
            <li>Small rottary table, controlled by a PLC</li>
            <li>Collection of sensors (camera, distance, force and induction)</li>
        </ul>

        <b>The station operates as follows:</b>
        The user creates a request via the HTTP interface, to produce an element with parts of 2 speciied colors.
        Then, the robot graps the parts from the corresponding rails and inspects with a distance sensor.
        The parts are put on the rottary table, and their color is checked by the camera.
        ...
        <br>
        <br>
        Throughout the entire process, the user can <b>preview the state of any sensors and internal signals in the HTTP interface</b>.
        <br>
        <br>
        The robot has communicates with ESP via the General Purpuse Input and Output ports. 
        Because ESP has to few of those, to fully implement state preview and reliable communication, 
        it was required to use additional <b>port extendors, on a I2C bus</b>.
        ...
        <br>
        <br>
        The communication other components (RaspberryPi and camera) are done via <b>MQTT protocol over WiFi</b>. The RaspberryPi acts as MQTT Broker.
        The PLC controlling the rottary table had to be controlled via <b>ModbusTCP</b>.
        <br>
        <br>
        The ASTORINO robot used in the project was still in the alpha developement stage. It made it so that, a lot of things were breaking all the time.
        For example, the most "fun" thing we encountered, was the robot reading an input pin as the inverse of it's accutal state, once every 10000 or so polls, 
        ...
        <li>Zbiór wielokolorowych elementów drukowanych w technologii 3D (podstawk i nakrętki)</li>
        <li>Mały stół obrotowy, sterowany przez PLC</li>
        <li>Zbiór czujników (kamera, odległości, siły i indukcjny)</li>
        </ul>
